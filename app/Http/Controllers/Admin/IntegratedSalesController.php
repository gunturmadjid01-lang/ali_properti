...
class IntegratedSalesController extends Controller
{
    public function index(Request $request, string $section = 'transactions'): Response
    {
        ...
        if (Schema::hasColumn($config['table'], 'record_status') && ! in_array($section, ['schemes', 'developer-products', 'contracts', 'developer-applications', 'bank-applications', 'bank-application-detail', 'bank-document-validation', 'bank-slik', 'bank-appraisal', 'bank-decision', 'bank-sp3k', 'bank-contract-preparation', 'bank-contract-schedule', 'bank-contract-execution', 'bank-disbursement', 'bank-change', 'bank-rejections', 'bank-reports'], true)) {
    ...
    public function show(Request $request, string $section, string $id): Response
    {
        ...
        [$kind,$record,$tabs] = match ($section) {
            ...
            'bank-applications', 'bank-application-detail', 'bank-document-validation', 'bank-slik', 'bank-appraisal', 'bank-decision', 'bank-sp3k', 'bank-contract-preparation', 'bank-contract-schedule', 'bank-contract-execution', 'bank-disbursement', 'bank-change', 'bank-rejections', 'bank-reports' => ['bank-application', KprSubmission::query()->with(['spr.costumer', 'spr.detailRumah.perumahan', 'spr.salesTransaction.processSteps.checklistItems', 'spr.salesTransaction.processSteps.documents', 'spr.salesTransaction.processSteps.assignee', 'bank', 'bankBranch', 'bankCreditProduct', 'stageHistories', 'financing', 'disbursements'])->findOrFail($id), ['Ringkasan', 'Proses sampai Huni', 'Dokumen', 'SLIK', 'Appraisal / Survei', 'Keputusan Bank', 'SP3K', 'Persiapan Akad', 'Jadwal & Pelaksanaan Akad', 'Pencairan', 'Perubahan Bank', 'Histori']],
            default => abort(404),
        };
        if ($kind === 'transaction') {
    ...
    private function printable(Request $request, string $section, string $id, bool $autoPrint)
    {
        ...
        [$kind, $record] = match ($section) {
            ...
            'bank-applications', 'bank-application-detail', 'bank-document-validation', 'bank-slik', 'bank-appraisal', 'bank-decision', 'bank-sp3k', 'bank-contract-preparation', 'bank-contract-schedule', 'bank-contract-execution', 'bank-disbursement', 'bank-change', 'bank-rejections', 'bank-reports' => ['bank-application', KprSubmission::query()->with(['spr.costumer', 'spr.detailRumah.perumahan', 'bank', 'bankBranch', 'bankCreditProduct', 'stageHistories', 'financing', 'disbursements'])->findOrFail($id)],
            default => abort(404),
        };
    ...
    private function configs(): array
    {
        ...
        $bankStatusLabel = DB::raw("CASE kpr_submissions.status WHEN 'pengumpulan_dokumen' THEN 'Pengumpulan Dokumen' WHEN 'slik_menunggu' THEN 'Menunggu SLIK' WHEN 'slik_lolos' THEN 'SLIK Lolos' WHEN 'analisa_diproses' THEN 'Analisis Diproses' WHEN 'appraisal' THEN 'Appraisal / Survei' WHEN 'disetujui' THEN 'Disetujui Bank' WHEN 'sp3k_keluar' THEN 'SP3K Keluar' WHEN 'akad' THEN 'Akad' WHEN 'pencairan_sebagian' THEN 'Pencairan Sebagian' WHEN 'pencairan_lunas' THEN 'Pencairan Lunas' WHEN 'selesai' THEN 'Selesai' WHEN 'ditolak' THEN 'Ditolak' ELSE kpr_submissions.status END as status_label");
        $bankBase = ['title' => 'Pengajuan KPR Bank', 'permission' => 'bank-kpr.applications', 'table' => 'kpr_submissions', 'editable' => false, 'fields' => [], 'joins' => [['sprs', 'sprs.id', '=', 'kpr_submissions.spr_id'], ['costumers', 'costumers.id', '=', 'sprs.costumer_id'], ['detail_rumahs', 'detail_rumahs.id', '=', 'sprs.detail_rumah_id'], ['perumahans', 'perumahans.id', '=', 'detail_rumahs.perumahan_id'], ['bank_kredits', 'bank_kredits.id', '=', 'kpr_submissions.bank_kredit_id'], ['users as handlers', 'handlers.id', '=', 'kpr_submissions.handled_by']], 'search' => ['kpr_submissions.kode_kpr', 'sprs.kode_spr', 'costumers.nama', 'bank_kredits.nama_bank']];
        $configs['bank-applications'] = [...$bankBase, 'columns' => [$col('kode_kpr', 'Nomor Pengajuan'), $col('kode_spr', 'Nomor SPR'), $col('customer_name', 'Customer'), $col('housing_name', 'Perumahan'), $col('unit_name', 'Unit'), $col('bank_name', 'Bank Tujuan'), $col('tanggal_pengajuan', 'Tanggal Pengajuan'), $col('status_label', 'Status')], 'select' => ['kpr_submissions.id', 'kpr_submissions.kode_kpr', 'sprs.kode_spr', 'costumers.nama as customer_name', 'perumahans.nama_perusahaan as housing_name', 'detail_rumahs.nomor_rumah as unit_name', 'bank_kredits.nama_bank as bank_name', 'kpr_submissions.tanggal_pengajuan', $bankStatusLabel]];
        $configs['bank-document-validation'] = [...$bankBase, 'title' => 'Proses KPR Bank', 'permission' => 'bank-kpr.document-validation', 'columns' => [$col('kode_kpr', 'Nomor Pengajuan'), $col('customer_name', 'Customer'), $col('bank_name', 'Bank'), $col('handler_name', 'PIC'), $col('nilai_pengajuan', 'Nilai Pengajuan'), $col('status_label', 'Tahap Saat Ini'), $col('last_stage_note', 'Catatan Terakhir')], 'select' => ['kpr_submissions.id', 'kpr_submissions.kode_kpr', 'costumers.nama as customer_name', 'bank_kredits.nama_bank as bank_name', 'handlers.name as handler_name', 'kpr_submissions.nilai_pengajuan', $bankStatusLabel, DB::raw('(SELECT catatan FROM kpr_stage_histories WHERE kpr_stage_histories.kpr_submission_id = kpr_submissions.id ORDER BY tanggal_status DESC, id DESC LIMIT 1) as last_stage_note')], 'conditions' => [['kpr_submissions.status', 'in', ['pengumpulan_dokumen', 'slik_menunggu', 'slik_lolos', 'analisa_diproses', 'appraisal', 'disetujui', 'ditolak']]]];
        $configs['bank-contract-schedule'] = [...$bankBase, 'title' => 'Akad & Pencairan KPR Bank', 'permission' => 'bank-kpr.contract-schedule', 'joins' => [...$bankBase['joins'], ['bank_kpr_financings', 'bank_kpr_financings.kpr_submission_id', '=', 'kpr_submissions.id']], 'columns' => [$col('kode_kpr', 'Nomor Pengajuan'), $col('customer_name', 'Customer'), $col('bank_name', 'Bank'), $col('sp3k_number', 'No. SP3K'), $col('approved_limit', 'Plafon Disetujui'), $col('expected_disbursement_date', 'Rencana Cair'), $col('disbursed_amount', 'Sudah Cair'), $col('status_label', 'Status')], 'select' => ['kpr_submissions.id', 'kpr_submissions.kode_kpr', 'costumers.nama as customer_name', 'bank_kredits.nama_bank as bank_name', 'bank_kpr_financings.sp3k_number', 'bank_kpr_financings.approved_limit', 'bank_kpr_financings.expected_disbursement_date', DB::raw('(SELECT COALESCE(SUM(amount), 0) FROM bank_kpr_disbursements WHERE bank_kpr_disbursements.kpr_submission_id = kpr_submissions.id AND bank_kpr_disbursements.deleted_at IS NULL AND bank_kpr_disbursements.status = "posted") as disbursed_amount'), $bankStatusLabel], 'conditions' => [['kpr_submissions.status', 'in', ['sp3k_keluar', 'akad', 'pencairan_sebagian', 'pencairan_lunas', 'selesai']]]];
        $configs['bank-reports'] = [...$bankBase, 'title' => 'Laporan KPR Bank', 'permission' => 'bank-kpr.reports', 'joins' => [...$bankBase['joins'], ['bank_kpr_financings', 'bank_kpr_financings.kpr_submission_id', '=', 'kpr_submissions.id']], 'columns' => [$col('kode_kpr', 'Nomor Pengajuan'), $col('customer_name', 'Customer'), $col('housing_name', 'Perumahan'), $col('bank_name', 'Bank'), $col('nilai_pengajuan', 'Nilai Pengajuan'), $col('approved_limit', 'Plafon'), $col('disbursed_amount', 'Total Pencairan'), $col('status_label', 'Status')], 'select' => ['kpr_submissions.id', 'kpr_submissions.kode_kpr', 'costumers.nama as customer_name', 'perumahans.nama_perusahaan as housing_name', 'bank_kredits.nama_bank as bank_name', 'kpr_submissions.nilai_pengajuan', 'bank_kpr_financings.approved_limit', DB::raw('(SELECT COALESCE(SUM(amount), 0) FROM bank_kpr_disbursements WHERE bank_kpr_disbursements.kpr_submission_id = kpr_submissions.id AND bank_kpr_disbursements.deleted_at IS NULL AND bank_kpr_disbursements.status = "posted") as disbursed_amount'), $bankStatusLabel]];
        $bankApplications = $configs['bank-applications'];
