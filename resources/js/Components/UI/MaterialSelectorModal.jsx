    ...
    return (
        <Modal open={open} onClose={onClose} title={title} size="xl">
            <div className="grid gap-3">
                <p className="text-sm font-semibold text-ink-soft dark:text-white/65">{description}</p>
                <Input
                    label="Cari Material"
                    value={search}
                ...
                <div className="max-h-[58vh] overflow-auto rounded-xl border border-silver-deep/60 dark:border-white/10">
                    <table className="w-full min-w-[860px] divide-y divide-silver-deep/60 text-xs">
                        <thead className="sticky top-0 z-10 bg-silver-soft/95 text-left uppercase tracking-[0.12em] text-ink-soft backdrop-blur dark:bg-[#232930] dark:text-white">
                            <tr>
                                {['Kode', 'Material', 'Jenis / Merk', 'Satuan', 'HPP', 'Aksi'].map((column) => (
                        ...
                        <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                            {filteredMaterials.map((material) => (
                                <tr key={material.value} className="hover:bg-silver-soft/60 dark:hover:bg-white/8">
                                    {(() => {
                                        const [kode, nama] = splitMaterialLabel(material.label);

                                        return (
                                            <>
                                                <td className="px-3 py-2 font-black text-ink dark:text-white">{material.kode_barang ?? kode ?? '-'}</td>
                                                <td className="px-3 py-2 font-bold">{material.nama_barang ?? nama ?? material.label ?? '-'}</td>
                                            </>
                                        );
                                    })()}
                                    <td className="px-3 py-2">{trimLabel(material)}</td>
                                    <td className="px-3 py-2">{material.satuan ?? '-'}</td>
                                    <td className="px-3 py-2 text-right font-black">{money(material.harga_hpp)}</td>
                                    <td className="px-3 py-2">
                                        <Button type="button" size="sm" onClick={() => onPick?.(material)}><PlusCircle size={15} /> Add</Button>
                                    </td>
                                </tr>
                            ))}
                            {filteredMaterials.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-4 py-10 text-center font-semibold text-ink-soft dark:text-white/55">
                ...
                <Button type="button" variant="outline" onClick={onClose}><X size={15} /> Tutup</Button>
                </div>
            </div>
        </Modal>
    );
