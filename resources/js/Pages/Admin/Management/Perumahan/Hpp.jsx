...
import { ArrowLeft, ChevronDown, Edit3, LoaderCircle, MinusCircle, Plus, Save, Trash2, XCircle } from 'lucide-react';
...
function isRequiredStage(group) {
    return String(group?.title ?? '').trim().toUpperCase() === 'IV RAB BANGUNAN';
}
...
function Hpp({
    ...
    const [draftRows, setDraftRows] = useState(() => rows.map(normalizeDraftRow));
    ...
    const pageSummary = useMemo(() => draftRows.reduce((carry, row) => {
        const jumlahRab = calculateAmount(row);
        const jumlahRealisasi = Number(row.jumlah_realisasi ?? 0);
        ...
        return carry;
    }, { jumlah_rab: 0, jumlah_realisasi: 0, sisa_anggaran: 0 }), [draftRows]);
    ...
    const stageGroups = useMemo(() => {
        ...
        (options?.tahapanHpps ?? []).forEach((stage) => {
            if (keyword && !String(stage.label ?? stage.nama_tahapan ?? '').toLowerCase().includes(keyword)) {
                return;
            }
        ...
        filteredRows.forEach((row) => {
            const key = row.tahapan_pembangunan_id || `none-${row.tahapan_nama ?? 'tanpa-tahap'}`;
    ...
    const updateDraftRow = (draftId, key, value) => {
        setDraftRows((currentRows) => currentRows.map((row) => (
            row.draft_id === draftId ? normalizeDraftRow({ ...row, [key]: value }) : row
        )));
    };
    ...
    const addItemToStage = (group) => {
        ...
        setDraftRows((currentRows) => {
            const stageRows = currentRows.filter((row) => (
                String(row.tahapan_pembangunan_id) === String(group.tahapan_pembangunan_id)
            ));
    ...
    const removeDraftItem = (row) => {
        if (!row.id) {
            setDraftRows((currentRows) => currentRows.filter((item) => item.draft_id !== row.draft_id));
            return;
        }
    ...
    const hppPayload = () => draftRows.map((row, index) => ({
        kelompok_hpp_id: row.kelompok_hpp_id ? String(row.kelompok_hpp_id) : '',
        tahapan_pembangunan_id: row.tahapan_pembangunan_id ? String(row.tahapan_pembangunan_id) : '',
        nama_pekerjaan: row.nama_pekerjaan ?? '',
        volume: row.volume ?? 0,
    ...
    const saveStageItems = (group) => {
        ...
        router.put(hppUrl, { items: hppPayload() }, {
            preserveScroll: true,
    ...
    const openEditStage = (group) => {
        const stage = (options?.tahapanHpps ?? []).find((item) => String(item.value) === String(group.tahapan_pembangunan_id));
    ...
    const deleteStage = (group) => {
        if (!window.confirm(`Hapus tahap "${group.title}" beserta ${group.rows.length} uraian pekerjaan di dalamnya?`)) {
            return;
        }

        router.delete(`${stageBaseUrl}/${group.tahapan_pembangunan_id}`, { preserveScroll: true });
    };

    const deleteItem = (row) => {
        if (!window.confirm(`Hapus uraian pekerjaan "${row.nama_pekerjaan}"?`)) {
            return;
        }
    ...
    return (
        <>
                ...
                <section className="rounded-lg border border-white/80 bg-white/78 p-6 shadow-soft dark:border-white/10 dark:bg-white/8">
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-3">
                        <div className="rounded-lg bg-silver-soft p-4 dark:bg-white/8">
                            ...
                            <p className="mt-1 text-xl font-extrabold">{money(pageSummary.jumlah_rab)}</p>
                        </div>
                        <div className="rounded-lg bg-silver-soft p-4 dark:bg-white/8">
                            <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-soft">Jumlah Realisasi</p>
                            <p className="mt-1 text-xl font-extrabold">{money(pageSummary.jumlah_realisasi)}</p>
                        </div>
                        <div className="rounded-lg bg-silver-soft p-4 dark:bg-white/8">
                            <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-soft">Sisa Anggaran</p>
                            <p className="mt-1 text-xl font-extrabold">{money(pageSummary.sisa_anggaran)}</p>
                        </div>
                    </div>
                </section>
                    ...
                    <div className="flex flex-col gap-3 border-b border-silver-deep/60 px-5 py-4 dark:border-white/10 md:flex-row md:items-center md:justify-between">
                        ...
                        {canCreateStructure && canManageStages && (
                            <div className="flex flex-wrap gap-2">
                                <Button variant="outline" size="sm" type="button" onClick={openCreateStage}>
                            ...
                            <div key={group.key}>
                                <div className="flex items-center gap-2 px-5 py-4 transition hover:bg-silver/60 dark:hover:bg-white/5">
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => setOpenStage(openStage === groupIndex ? null : groupIndex)}
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-extrabold text-ink dark:text-white">{group.title}</p>
                                                <p className="text-xs font-bold text-ink-soft dark:text-white/50">{group.rows.length} pekerjaan</p>
                                            </div>
                                        </div>
                                        <div className="flex flex-wrap items-center gap-2 text-xs font-extrabold text-ink-soft dark:text-white/60">
                                            <span>RAB {money(group.jumlah_rab)}</span>
                                            <span>Realisasi {money(group.jumlah_realisasi)}</span>
                                            <span>Sisa {money(group.sisa_anggaran)}</span>
                                            <ChevronDown className={`transition ${openStage === groupIndex ? 'rotate-180' : ''}`} size={17} />
                                        </div>
                                    </button>
                                    {canManageStructure && canManageStages && group.tahapan_pembangunan_id && (
                                        <div className="flex shrink-0 gap-1">
                                            {canUpdateStructure && !isAutomaticStage(group, hppContext) && (
                                                <Button variant="ghost" size="sm" type="button" onClick={() => openEditStage(group)} title="Edit tahap dan urutan">
                                                    <Edit3 size={15} />
                                                </Button>
                                            )}
                                            {canDeleteStructure && !isRequiredStage(group) && (
                                                <Button variant="ghost" size="sm" type="button" onClick={() => deleteStage(group)} title="Hapus tahap">
                                                    <Trash2 size={15} />
                                                </Button>
                                            )}
                                        </div>
                                ...
                                {openStage === groupIndex && (
                                    <div className="px-5 pb-5">
                                        {canCreateStructure && !isAutomaticStage(group, hppContext) && (
                                            <div className="mb-3 flex justify-end">
                                                <Button
                                                    size="sm"
                                                    variant="outline"
                                                    type="button"
                                                    onClick={() => addItemToStage(group)}
                                                >
                                                    <Plus size={15} /> Tambah Item Tahap Ini
                                                </Button>
                                            </div>
                                        )}
                                        <div className="overflow-x-auto rounded-lg border border-silver-deep/60 dark:border-white/10">
                                            <table className="min-w-full divide-y divide-silver-deep/60 text-xs dark:divide-white/10">
                                                <thead className="bg-silver-soft/80 text-left text-xs uppercase tracking-[0.12em] text-ink-soft dark:bg-white/5 dark:text-white/50">
                                                    <tr>
                                                        {['No', 'Nama Pekerjaan', 'Volume', 'Satuan', 'Harga Satuan', 'Total', 'Jumlah Realisasi', 'Sisa Anggaran', ...(canDeleteStructure && !isAutomaticStage(group, hppContext) ? ['Aksi'] : [])].map((column) => (
                                                ...
                                                <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                                                    {group.rows.map((row, index) => (
                                                        <tr className="transition hover:bg-silver/70 dark:hover:bg-white/5" key={row.draft_id ?? (row.id ? `hpp-item-${row.id}` : `${group.key}-${index}`)}>
                                                            <td className="px-4 py-3 font-semibold">{index + 1}</td>
                                                            <td className="min-w-72 px-4 py-3 font-semibold text-ink dark:text-white">
                                                                {canManageValues && !isAutomaticStage(group, hppContext) ? (
                                                                    <Input
                                                                        value={row.nama_pekerjaan}
                                                                        onChange={(event) => updateDraftRow(row.draft_id, 'nama_pekerjaan', event.target.value)}
                                                                        placeholder="Nama pekerjaan"
                                                                    />
                                                                ) : (row.nama_pekerjaan ?? row.kelompok_hpp_nama ?? '-')}
                                                            </td>
                                                            <td className="min-w-32 px-4 py-3 font-semibold">
                                                                {canManageValues && !isAutomaticStage(group, hppContext) ? (
                                                                    <Input
                                                                        ...
                                                                        step="0.01"
                                                                        value={row.volume}
                                                                        onChange={(event) => updateDraftRow(row.draft_id, 'volume', event.target.value)}
                                                                    />
                                                            ...
                                                            <td className="min-w-28 px-4 py-3 font-semibold">
                                                                {canManageValues && !isAutomaticStage(group, hppContext) ? (
                                                                    <Input
                                                                        value={row.satuan}
                                                                        onChange={(event) => updateDraftRow(row.draft_id, 'satuan', event.target.value)}
                                                            ...
                                                            <td className="min-w-44 px-4 py-3 font-semibold">
                                                                {canManageValues && !isAutomaticStage(group, hppContext) ? (
                                                                    <CurrencyInput
                                                                        ...
                                                                        onChange={(value) => updateDraftRow(row.draft_id, 'harga_satuan', value)}
                                                                    />
                                                                ) : money(row.harga_satuan)}
                                                            </td>
                                                            ...
                                                            {canDeleteStructure && !isAutomaticStage(group, hppContext) && (
                                                                <td className="px-4 py-3">
                                                                    <div className="flex gap-1">
                                                                        <Button variant="ghost" size="sm" type="button" onClick={() => removeDraftItem(row)} title="Hapus uraian">
                                                                            <Trash2 size={15} />
                                                                        </Button>
                                                                    </div>
                                                                </td>
                                                    ...
                                                    <tr className="bg-silver-soft/80 font-extrabold dark:bg-white/5">
                                                        <td className="px-4 py-3 text-right" colSpan={5}>SUB TOTAL {group.title}</td>
                                                        <td className="px-4 py-3">{money(group.jumlah_rab)}</td>
                                                        <td className="px-4 py-3">{money(group.jumlah_realisasi)}</td>
                                                        <td className="px-4 py-3">{money(group.sisa_anggaran)}</td>
                                                        {canDeleteStructure && !isAutomaticStage(group, hppContext) && <td />}
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        {canManageValues && !isAutomaticStage(group, hppContext) && (
                                            <div className="mt-4 flex justify-end">
                                                <Button type="button" onClick={() => saveStageItems(group)} disabled={savingStage === group.key}>
                                                    {savingStage === group.key ? <LoaderCircle className="animate-spin" size={17} /> : <Save size={17} />}
                                                    {savingStage === group.key ? 'Menyimpan...' : `Simpan ${group.title}`}
                                                </Button>
                                            </div>
                                        )}
                                    </div>
            ...
            {stageOpen && (editingStage ? canUpdateStructure : canCreateStructure) && (
                <ModalForm
                    open={stageOpen}
                    onClose={() => {
                        setStageOpen(false);
                    ...
                    title={`${editingStage ? 'Edit' : 'Tambah'} Tahap HPP ${hppContext === 'unit' ? 'Unit' : 'Kawasan'}`}
                    description="Tentukan nama tahap dan posisinya pada urutan accordion HPP."
                    onSubmit={submitStage}
                    actions={
                        <>
                            <Button variant="outline" type="button" onClick={() => setStageOpen(false)}>
                                <XCircle size={17} /> Batal
                            </Button>
                            <Button type="submit" disabled={stageForm.processing}>
                                <Plus size={17} /> {stageForm.processing ? 'Menyimpan...' : (editingStage ? 'Simpan Tahap' : 'Tambah Tahap')}
                            </Button>
                        </>
                    }
                >
                    <div className="grid gap-3 md:grid-cols-2">
                        <Input label="Nama Tahap" value={stageForm.data.nama_tahapan} onChange={(event) => stageForm.setData('nama_tahapan', event.target.value)} />
                        <Input label="Urutan" type="number" value={stageForm.data.urutan} onChange={(event) => stageForm.setData('urutan', event.target.value)} />
                    </div>
                    {!editingStage && (
                        <div className="grid gap-3 rounded-lg border border-silver-deep/60 p-4 dark:border-white/10">
                            ...
                            {(stageForm.data.items ?? []).map((item, index) => (
                                <div className="grid gap-3 rounded-lg bg-silver-soft/80 p-3 dark:bg-white/5 md:grid-cols-2 xl:grid-cols-[1.6fr_0.65fr_0.65fr_0.9fr_auto]" key={index}>
                                    ...
                                    <Input label="Harga Satuan" type="number" value={item.harga_satuan} onChange={(event) => stageForm.setData('items', stageForm.data.items.map((row, rowIndex) => rowIndex === index ? { ...row, harga_satuan: event.target.value } : row))} />
                    ...
                    {stageForm.errors.nama_tahapan && <span className="text-xs font-bold text-red-600">{stageForm.errors.nama_tahapan}</span>}
                </ModalForm>
            )}
        </>
    );
