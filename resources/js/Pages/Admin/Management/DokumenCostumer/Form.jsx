...
export default function DokumenCostumerForm({ open, title, fields, options, form, selected, onSubmit, onClose }) {
    return (
        <ModalForm
            open={open}
            onClose={onClose}
            title={selected ? `Edit ${title}` : `Tambah ${title}`}
            description="Atur dokumen wajib atau opsional untuk proses SPR, KPR, cash, dan bertahap."
            onSubmit={onSubmit}
