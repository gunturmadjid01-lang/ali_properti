    ...
    return (
        <>
            ...
            <div className="grid gap-4">
                <section className="overflow-hidden rounded-2xl border border-white/70 bg-white/85 shadow-soft dark:border-white/10 dark:bg-white/6">
                    <div className="flex flex-col gap-3 border-b border-silver-deep/50 px-5 py-4 dark:border-white/10 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-ink-soft dark:text-white/45">
                                Master Logistik
                            </p>
                            <h1 className="mt-1 text-xl font-black text-ink dark:text-white">
                                Kelola Supplier
                            </h1>
                        </div>
                        ...
                        <table className="w-full min-w-[1040px] divide-y divide-silver-deep/60 text-xs">
                            ...
                            <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                                {rows.data.map((row) => (
                                    <tr
                                        key={row.id}
                                        className="hover:bg-silver-soft/70 dark:hover:bg-white/8"
                                    >
                                        <td className="px-4 py-3 font-black text-ink dark:text-white">
                                            {row.kode_supplier}
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="font-black text-ink dark:text-white">
                                                {row.nama_supplier}
                                            </div>
                                            <div className="mt-1 max-w-[260px] truncate text-[11px] font-semibold text-ink-soft dark:text-white/50">
                                                {row.alamat || "-"}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-[11px] text-ink-soft dark:text-white/50">
                                                {row.phone || row.email || "-"}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3">
                                            <span
                                                className={`rounded-full px-3 py-1 text-[11px] font-black ${row.status === "aktif" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-200" : "bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-white/60"}`}
                                            >
                                                {row.status}
                                            </span>
                                        </td>
            ...
            <Modal
                ...
                title={
                    editing ? `Edit ${editing.nama_supplier}` : "Supplier Baru"
                }
                size="lg"
                footer={
                    <>
                ...
                >
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <Input
                            ...
                            onChange={(event) =>
                                form.setData("pic", event.target.value)
                            }
                        />
                        <Input
                            label="No. Telepon"
                        ...
                        <Input
                            label="Bank"
                            value={form.data.nama_bank}
                            error={form.errors.nama_bank}
                            onChange={(event) =>
                                form.setData("nama_bank", event.target.value)
                            }
                        />
                        <Input
                            label="Nomor Rekening"
                            value={form.data.nomor_rekening}
                            error={form.errors.nomor_rekening}
                            onChange={(event) =>
                                form.setData(
                                    "nomor_rekening",
                                    event.target.value,
                        ...
                        <Input
                            ...
                            onChange={(event) =>
                                form.setData("npwp", event.target.value)
                            }
                        />
                    </div>
                    <label className="grid gap-2 text-sm font-extrabold text-ink/75 dark:text-white/78">
...
Supplier.layout = (page) => (
    <AdminLayout title={page?.props?.title ?? "Kelola Supplier"}>
        {page}
    </AdminLayout>
);
