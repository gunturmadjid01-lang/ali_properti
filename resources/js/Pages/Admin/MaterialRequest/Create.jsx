...
function itemTemplate() {
    return { barang_material_id: '', kode_barang: '', nama_barang: '', satuan: '', qty: '', catatan: '' };
}
...
export default function Create({ title, baseUrl, indexUrl, nextCode = '', request = null, options = {}, canCreate = false }) {
    ...
    const visibleItems = useMemo(() => {
        ...
        return form.data.items
            ...
            .filter(({ item }) => [item.kode_barang, item.nama_barang, item.satuan, item.catatan].filter(Boolean).join(' ').toLowerCase().includes(keyword));
    ...
    const setItem = (index, key, value, selected) => {
        form.setData('items', form.data.items.map((item, itemIndex) => {
            ...
            if (key === 'barang_material_id') {
                ...
                next.kode_barang = selected?.kode_barang ?? kode ?? next.kode_barang;
                next.nama_barang = selected?.nama_barang ?? nama ?? selected?.label ?? next.nama_barang;
                next.satuan = selected?.satuan ?? '';
            }
            return next;
        }));
    };
    ...
    const pickMaterial = (material) => {
        ...
        if (materialPicker.index !== null) {
            setItem(materialPicker.index, 'barang_material_id', material.value, material);
        }

        closePicker();
    };

    const totalQty = form.data.items.reduce((sum, item) => sum + Number(item.qty || 0), 0);
        ...
        <>
            ...
            <form className="overflow-hidden rounded-2xl border border-white/70 bg-white/85 shadow-soft dark:border-white/10 dark:bg-white/6" onSubmit={submit}>
                <div className="grid gap-3 border-b border-silver-deep/50 bg-silver-soft/55 px-4 py-3 dark:border-white/10 dark:bg-white/4 xl:grid-cols-[260px_180px_1fr_220px]">
                    <Input label="No Transaksi" value={form.data.kode_request} readOnly inputClassName="h-9 min-h-9 cursor-not-allowed bg-silver-soft text-xs dark:bg-white/5" />
                ...
                <div className="grid gap-3 border-b border-silver-deep/50 bg-silver-soft/35 px-4 py-3 dark:border-white/10 dark:bg-white/3 lg:grid-cols-3">
                    <SummaryCard title="Perumahan" value={options.perumahans?.find((row) => row.value === String(form.data.perumahan_id))?.label ?? 'Opsional'} />
                    <SummaryCard title="Unit Rumah" value={detailRumahOptions.find((row) => row.value === String(form.data.detail_rumah_id))?.label ?? 'Opsional'} />
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                        ...
                        <label className="grid gap-2 text-sm font-extrabold text-ink/75 dark:text-white/78">
                            ...
                            <Dropdown
                                ...
                                onChange={(value, selected) => form.setData({ ...form.data, detail_rumah_id: value, perumahan_id: selected?.perumahan_id ?? form.data.perumahan_id })}
                                buttonClassName="min-h-9 text-xs"
                            />
                        </label>
                        <Input value={search} onChange={(event) => setSearch(event.target.value)} label="Cari Item" inputClassName="h-9 min-h-9 text-xs" />
                    </div>
                </div>
                ...
                <div className="h-[58vh] overflow-auto">
                    <table className="w-full min-w-[1050px] divide-y divide-silver-deep/60 text-xs">
                        <thead className="sticky top-0 z-10 bg-silver-soft/95 text-left uppercase tracking-[0.12em] text-ink-soft backdrop-blur dark:bg-[#232930] dark:text-white">
                            <tr>
                                {['No', 'Kode Item', 'Material', 'Qty', 'Satuan', 'Catatan', ''].map((column) => (
                        ...
                        <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                            {visibleItems.map(({ item, index }) => (
                                <tr key={`${item.barang_material_id || 'new'}-${index}`}>
                                    ...
                                    <td className="px-3 py-2">
                                        <input
                                            value={item.satuan}
                                            onChange={(event) => setItem(index, 'satuan', event.target.value)}
                                    ...
                                    <td className="px-3 py-2">
                                        <input
                                            value={item.catatan}
                                            onChange={(event) => setItem(index, 'catatan', event.target.value)}
                            ...
                            {visibleItems.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-4 py-10 text-center font-semibold text-ink-soft dark:text-white/55">Tidak ada item material yang cocok dengan pencarian ini.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="grid gap-3 border-t border-silver-deep/60 bg-silver-soft/55 px-4 py-3 dark:border-white/10 dark:bg-white/4 lg:grid-cols-[1fr_360px]">
                    <div className="flex flex-wrap items-end gap-2">
                        ...
                        <Button type="submit" size="sm" disabled={form.processing || !canSubmit}><Save size={16} /> Simpan</Button>
                        <Button type="button" variant="outline" size="sm" onClick={() => router.visit(indexUrl)}><RefreshCw size={16} /> Batal</Button>
                    </div>
                    <div className="grid gap-2">
                        <FooterTotal label="Total Item" value={form.data.items.length} />
                        <FooterTotal label="Total Qty" value={totalQty} strong />
                    </div>
                </div>
            </form>
...
function SummaryCard({ title, value }) {
    return (
        <div className="rounded-xl border border-silver-deep/60 bg-white/70 p-4 dark:border-white/10 dark:bg-white/5">
            ...
            <p className="mt-2 text-lg font-black text-ink dark:text-white">{value}</p>
        </div>
    );
}
...
function FooterTotal({ label, value, strong = false }) {
    return (
        <div className={`grid grid-cols-[1fr_160px] items-center gap-3 text-xs ${strong ? 'font-black' : 'font-bold'}`}>
...
Create.layout = (page) => <AdminLayout title={page?.props?.title ?? 'Permintaan Barang'}>{page}</AdminLayout>;
