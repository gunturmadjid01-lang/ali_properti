...
import { Button, Input, Modal, TableActions } from "../../../Components/UI";
...
export default function Index({
    ...
    const lockRow = (row) =>
        window.confirm(`Lock permintaan ${row.kode_request}?`) &&
        router.post(`${baseUrl}/${row.id}/lock`, {}, { preserveScroll: true });
    const unlockRow = (row) =>
        window.confirm(`Buka lock permintaan ${row.kode_request}?`) &&
        router.post(
    ...
    const approveOwner = (row) =>
        window.confirm(`Approve owner untuk ${row.kode_request}?`) &&
        router.post(
            `${baseUrl}/${row.id}/approve-owner`,
            {},
            { preserveScroll: true },
        );
    const issueRow = (row) =>
        window.confirm(`Kirim barang untuk ${row.kode_request}?`) &&
        router.post(`${baseUrl}/${row.id}/issue`, {}, { preserveScroll: true });
            ...
            <>
                {row.record_status === "locked" ? (
                    <>
                        ...
                        {canApproveOwner && row.can_approve_owner && (
                            <IconButton
                                title="Setujui Owner"
                                icon={CheckCircle2}
                                onClick={() => approveOwner(row)}
                            />
                        )}
                        {canIssue && row.can_issue && (
                            <IconButton
                                title="Kirim Barang"
                                icon={Send}
                                onClick={() => issueRow(row)}
                            />
                        )}
                        {row.can_unlock && canUnlock && (
                            <IconButton
                                title="Buka Kunci"
                                icon={Unlock}
        ...
        <>
            ...
            <section className="overflow-hidden rounded-2xl border border-white/70 bg-white/85 shadow-soft dark:border-white/10 dark:bg-white/6">
                <div className="border-b border-silver-deep/50 bg-silver-soft/55 px-4 py-3 dark:border-white/10 dark:bg-white/4">
                    <form
                        className="grid gap-2 xl:grid-cols-[1fr_auto]"
                    ...
                    <div className="grid gap-4">
                        <div className="grid gap-3 rounded-lg border border-silver-deep/60 bg-silver-soft/60 p-4 dark:border-white/10 dark:bg-white/5 md:grid-cols-5">
                            ...
                            <div>
                                ...
                                <p className="mt-1 font-black">{detail.unit}</p>
                            </div>
                            <div>
                                <p className="text-xs font-bold uppercase text-ink-soft">
                                    Status
                            ...
                            <table className="w-full min-w-[860px] divide-y divide-silver-deep/60 text-xs">
                                ...
                                <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                                    {detail.items.map((item, index) => (
                                        <tr
                                            key={`${item.barang_material_id}-${index}`}
                                        >
                                            <td className="px-3 py-2 font-bold">
                                                {index + 1}
                                            </td>
