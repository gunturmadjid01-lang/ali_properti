        ...
        >
            ...
            >
                <Input
                    ...
                    onChange={(event) =>
                        form.setData("tanggal_pembayaran", event.target.value)
                    }
                />
                <CurrencyInput
                    label="Nominal"
                    ...
                    onChange={(value) => form.setData("nominal", value)}
                />
...
export default function Index({
    ...
    const selectedSpr = useMemo(
        () =>
            options.sprOptions?.find(
                (item) => item.value === form.data.spr_id,
            ) ?? null,
        ...
        <>
            ...
            <div className="grid gap-6">
                ...
                <Form
                    ...
                    description="Pilih SPR cash yang sudah disetujui. Data pelanggan, unit, dan harga akan ikut terbaca otomatis."
                    onSubmit={submit}
                    actions={
                        <Button type="submit" disabled={form.processing}>
                            {form.processing ? (
                ...
                >
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        <div className="grid gap-2 md:col-span-2">
                            ...
                            <Dropdown
                                ...
                                label="Pilih SPR Tunai"
                                options={options.sprOptions ?? []}
                                onChange={(value) =>
                                    form.setData("spr_id", value)
                                }
                            />
                            {form.errors.spr_id && (
                                <span className="text-xs font-bold text-red-600">
                    ...
                    {selectedSpr && (
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            {[
                                ["Pelanggan", selectedSpr.customer],
                                ...
                                [
                                    "Sisa Sementara",
                                    money(selectedSpr.sisa_sementara),
                                ],
                            ].map(([label, value]) => (
                                <div
            ...
            <DetailModal
                ...
                title={
                    detailRow
                        ? `Detail Cash ${detailRow.kode_cash}`
                        : "Detail Tunai"
                }
                columns={[
                    { key: "kode_cash", label: "Kode Tunai" },
                    ...
                    { key: "perumahan", label: "Perumahan" },
                    {
                        key: "harga_rumah",
                        label: "Harga Rumah",
                        render: (row) => money(row.harga_rumah),
                    },
                    {
                        key: "total_dibayar",
                        label: "Total Dibayar",
                        render: (row) => money(row.total_dibayar),
                    },
                    {
                        key: "sisa_tagihan",
                        label: "Sisa Tagihan",
                        render: (row) => money(row.sisa_tagihan),
                    },
...
Index.layout = (page) => (
    <AdminLayout title={page?.props?.title ?? "Transaksi Tunai"}>
        {page}
    </AdminLayout>
);
