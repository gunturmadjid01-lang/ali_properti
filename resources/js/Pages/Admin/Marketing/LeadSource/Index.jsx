...
import { Edit3, Eye, Lock, PlusCircle, Search, Trash2, Unlock, X } from 'lucide-react';
...
export default function Index({ title, description, baseUrl, rows = { data: [], links: [] }, filters = {}, options = {}, permissions = {} }) {
    ...
    return (
        <>
            ...
            <div className="grid gap-6">
                ...
                <section className="overflow-hidden rounded-lg border border-white/80 bg-white/78 shadow-soft dark:border-white/10 dark:bg-white/8">
                    <form className="grid gap-3 p-5 md:grid-cols-[1fr_auto]" onSubmit={(event) => { event.preventDefault(); router.get(baseUrl, { search }, { preserveState: true, replace: true }); }}>
                        <Input label="Cari Kode / Nama / Kategori" value={search} onChange={(event) => setSearch(event.target.value)} />
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead className="bg-silver-soft/80 text-left text-xs uppercase tracking-wider text-ink-soft">
                                <tr>{['Kode', 'Nama Sumber', 'Kategori', 'Customer', 'Status', 'Audit', 'Lock', 'Aksi'].map((label) => <th className="px-5 py-4" key={label}>{label}</th>)}</tr>
                            </thead>
                            <tbody className="divide-y divide-silver-deep/50">
                                {rows.data.map((row) => (
                                    <tr key={row.id}>
                                        <td className="px-5 py-4 font-bold">{row.kode_sumber}</td>
                                        <td className="px-5 py-4 font-extrabold">{row.nama_sumber}</td>
                                        <td className="px-5 py-4">{row.kategori}</td>
                                        <td className="px-5 py-4">{row.jumlah_customer}</td>
                                ...
                                {rows.data.length === 0 && <tr><td className="px-5 py-10 text-center font-bold text-ink-soft" colSpan={8}>Belum ada sumber lead.</td></tr>}
                            </tbody>
                        </table>
                    </div>
            ...
            <Modal open={Boolean(detail)} onClose={() => setDetail(null)} title={detail ? `Detail ${detail.kode_sumber}` : 'Detail Sumber Lead'} footer={<Button type="button" variant="outline" onClick={() => setDetail(null)}>Tutup</Button>}>
                {detail && (
                    <div className="grid gap-3 text-sm">
                        <p><b>Nama:</b> {detail.nama_sumber}</p>
                        <p><b>Kategori:</b> {detail.kategori}</p>
                        <p><b>Status:</b> {detail.status}</p>
                        <p><b>Jumlah Customer:</b> {detail.jumlah_customer}</p>
...
Index.layout = (page) => <AdminLayout title={page?.props?.title ?? 'Sumber Lead'}>{page}</AdminLayout>;
