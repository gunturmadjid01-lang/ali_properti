...
const badge = (status) => {
    const danger = ["ditolak", "dibatalkan", "jatuh_tempo", "revisi"].includes(
        status,
    );
    const success = [
        "aktif",
    ...
    return `rounded-full px-3 py-1 text-xs font-extrabold ${
        danger
...
function Card({ children, className = "" }) {
    return (
        <section
            className={`rounded-2xl border border-white/80 bg-white/80 shadow-soft dark:border-white/10 dark:bg-white/7 ${className}`}
        >
            {children}
        </section>
    );
...
function Empty({ text = "Belum ada data." }) {
    return (
        <div className="px-5 py-10 text-center text-sm font-bold text-ink-soft dark:text-white/50">
            {text}
        </div>
    );
}
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {stats.map(([label, value, Icon]) => (
                    <Card className="p-5" key={label}>
                        ...
                        <p className="mt-3 text-sm font-bold text-ink-soft dark:text-white/55">
                            {label}
                        </p>
                    </Card>
                ))}
                ...
                <Card>
                    ...
                    <div className="divide-y divide-silver-deep/50 dark:divide-white/10">
                        ...
                        {(data.performance ?? []).length === 0 && (
                            <Empty text="Belum ada aktivitas marketing bulan ini." />
                        )}
                    </div>
                </Card>
                <Card>
...
function KampanyeTable({ data, onEdit, onDelete, onLock, permissions = {} }) {
    return (
                    ...
                    <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                        {(data.rows ?? []).map((row) => (
                            <tr key={row.id}>
                                ...
                                <td className="px-4 py-4">
                                    {money(row.realisasi_biaya)}
                                </td>
                                <td className="px-4 py-4">
                                    {row.lead_count} / {row.target_lead}
                                </td>
                                <td className="px-4 py-4">
                                    <span className={badge(row.status)}>
                                        {row.status}
    ...
    return (
        <Card className="overflow-hidden">
            <div className="divide-y divide-silver-deep/50 dark:divide-white/10">
                {(data.rows ?? []).map((row) => (
                    <div
                        className={`grid gap-4 p-5 md:grid-cols-[1.2fr_1fr_180px_auto] md:items-center ${row.is_overdue ? "bg-red-50/70 dark:bg-red-500/5" : ""}`}
                        key={row.id}
                    >
                        <div>
                            <strong>{row.judul}</strong>
                            <p className="text-xs text-ink-soft">
                                {row.customer} · {row.telepon}
                            </p>
                        </div>
                        <div className="text-sm">
                            <b>{row.remind_at?.replace("T", " ")}</b>
                            <p className="text-xs text-ink-soft">{row.user}</p>
                        </div>
                ...
                >
                    <form className="grid gap-4" onSubmit={submit}>
                        ...
                        <Textarea
                            ...
                            value={form.data.catatan_revisi}
                            onChange={(e) =>
                                form.setData("catatan_revisi", e.target.value)
                            }
                        />
    ...
    return (
        <div className="grid gap-6">
            ...
            <Card className="overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                        <thead className="bg-silver-soft text-left text-xs uppercase text-ink-soft dark:bg-white/5">
                            <tr>
                                {[
                                    "SPR",
                                    "Pelanggan",
            ...
            <Card className="overflow-hidden">
                ...
                <div className="divide-y divide-silver-deep/50 dark:divide-white/10">
                    {(data.receipts ?? []).map((row) => (
                        <div
                            className="grid gap-3 px-5 py-4 md:grid-cols-[160px_1fr_150px_180px_auto] md:items-center"
                            key={row.id}
                        >
                            <b>{row.nomor}</b>
                            <span>
                                {row.customer}
                                <br />
                                <small>{row.kode_spr}</small>
                            </span>
                            <span>{row.tanggal}</span>
                            <b>{money(row.nominal)}</b>
                            <Button
                                as="a"
                                href={row.print_url}
                                target="_blank"
                        ...
                        <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                            {(data.targets ?? []).map((row) => (
                                <tr key={row.id}>
                                    ...
                                    <td className="px-4 py-4">
                                        {money(row.target_nilai_penjualan)}
                                    </td>
                                    <td className="px-4 py-4">
                                        <Actions
                                            row={row}
                    ...
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p className="text-xs uppercase text-ink-soft">
                                {row.kanal} · {row.tahapan || "umum"}
                            </p>
                        </div>
                        <span className={badge(row.status)}>{row.status}</span>
                    </div>
                    <p className="mt-4 min-h-24 whitespace-pre-wrap rounded-xl bg-silver-soft/80 p-3 text-sm dark:bg-white/5">
                        {row.isi_template}
                    </p>
...
function CrudModal({ section, row, type, options, baseUrl, onClose }) {
    ...
    const form = useForm(
        row ? { ...defaults, ...row, type: type ?? row.type } : defaults,
    );
    const submit = (event) => {
        event.preventDefault();
        const url = row ? `${baseUrl}/${row.id}` : baseUrl;
        form.transform((data) => ({ ...data, type: type ?? data.type }));
        (row ? form.put : form.post)(url, {
            preserveScroll: true,
            onSuccess: onClose,
        });
    };
    const spr = options.sprs?.find(
        (item) => String(item.value) === String(form.data.spr_id),
    );
    return (
        <Modal
            open
            onClose={onClose}
            title={`${row ? "Ubah" : "Tambah"} ${type === "commission" ? "Komisi" : type === "target" ? "Target" : ""}`}
            size="lg"
        >
            <form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
                {section === "campaign" && (
                    <>
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("nama_campaign", e.target.value)
                            }
                        />
                        <Input
                            label="Kanal Promosi"
                            ...
                            onChange={(e) =>
                                form.setData("kanal", e.target.value)
                            }
                        />
                        <Input
                            type="date"
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("tanggal_selesai", e.target.value)
                            }
                        />
                        <CurrencyInput
                            label="Anggaran"
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("target_lead", e.target.value)
                            }
                        />
                        <Dropdown
                            label="Status"
                ...
                {section === "reminder" && (
                    <>
                        <Dropdown
                            label="Pelanggan"
                            value={String(form.data.costumer_id ?? "")}
                            options={options.customers ?? []}
                            onChange={(v) => form.setData("costumer_id", v)}
                        />
                        <Dropdown
                            label="Petugas"
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("jenis", e.target.value)
                            }
                        />
                        <Input
                            label="Judul"
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("remind_at", e.target.value)
                            }
                        />
                        <Dropdown
                            label="Status"
                            value={form.data.status}
                            options={["menunggu", "selesai", "dibatalkan"].map(
                                (x) => ({ value: x, label: x }),
                            )}
                            onChange={(v) => form.setData("status", v)}
                        />
                ...
                {section === "template" && (
                    <>
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("nama_template", e.target.value)
                            }
                        />
                        <Dropdown
                            label="Kanal"
                            value={form.data.kanal}
                            options={["whatsapp", "sms", "email"].map((x) => ({
                                value: x,
                                label: x,
                            }))}
                            onChange={(v) => form.setData("kanal", v)}
                        />
                        <Input
                            label="Tahapan"
                            value={form.data.tahapan}
                            onChange={(e) =>
                                form.setData("tahapan", e.target.value)
                            }
                        />
                        <Dropdown
                            label="Status"
                            value={form.data.status}
                            options={["aktif", "nonaktif"].map((x) => ({
                                value: x,
                                label: x,
                            }))}
                            onChange={(v) => form.setData("status", v)}
                        ...
                        <Textarea
                            ...
                            onChange={(e) =>
                                form.setData("isi_template", e.target.value)
                            }
                        />
                    </>
                )}
                {section === "target-komisi" && type === "target" && (
                    <>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("tahun", e.target.value)
                            }
                        />
                        <Input
                            type="number"
                            ...
                            onChange={(e) =>
                                form.setData("bulan", e.target.value)
                            }
                        />
                        {[
                            "target_lead",
                        ...
                        <CurrencyInput
                            ...
                            onChange={(v) =>
                                form.setData("target_nilai_penjualan", v)
                            }
                        />
                    </>
                )}
                {section === "target-komisi" && type === "commission" && (
                    <>
                        <Dropdown
                            ...
                            onChange={(v) => {
                                ...
                                form.setData((data) => ({
                                    ...
                                    dasar_perhitungan:
                                        selected?.amount ??
                                        data.dasar_perhitungan,
                                    user_id: selected?.user_id ?? data.user_id,
                                }));
                            }}
                        />
                        <Dropdown
                        ...
                        <CurrencyInput
                            ...
                            value={
                                form.data.dasar_perhitungan || spr?.amount || ""
                            }
                            onChange={(v) =>
                                form.setData("dasar_perhitungan", v)
                            }
                        />
                        <Input
                            type="number"
                            ...
                            onChange={(e) =>
                                form.setData("persentase", e.target.value)
                            }
                        />
                        <Dropdown
                            label="Status"
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                form.setData("tanggal_dibayar", e.target.value)
                            }
                        />
                    </>
                )}
...
export default function Index({
    ...
    const lockRow = (row, type) => {
        ...
        router.post(
            `/admin/marketing/operasional/${modelSection}/${row.id}/${action}`,
            {},
            { preserveScroll: true },
        );
    };
                        ...
                        <div className="flex flex-wrap gap-2">
                            {section === "dashboard" && (
                                <Button
                                    ...
                                    onClick={() =>
                                        router.post(
                                            "/admin/marketing/operasional/booking-expired/process",
                                        )
                                    }
                                >
                                    <AlarmClock size={16} /> Proses Booking
                                    Expired
                                </Button>
                            )}
...
Index.layout = (page) => (
    <AdminLayout title={page?.props?.title ?? "Marketing Workspace"}>
        {page}
    </AdminLayout>
);
