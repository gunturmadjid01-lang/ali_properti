...
import { Edit3, Eye, Lock, MinusCircle, PackageCheck, PlusCircle, Search, Trash2, Unlock, X } from 'lucide-react';
...
export default function Index({ title, baseUrl, rows = { data: [], links: [] }, filters = {}, options = {}, siteStockRows = [] }) {
    ...
    const stockOptions = useMemo(() => (options.siteStocks ?? []).filter((row) => (
        ...
        && (!form.data.detail_rumah_id || row.detail_rumah_id === String(form.data.detail_rumah_id))
        && (!form.data.tahapan_pembangunan_id || row.tahapan_pembangunan_id === String(form.data.tahapan_pembangunan_id))
    )), [form.data.perumahan_id, form.data.detail_rumah_id, form.data.tahapan_pembangunan_id, options.siteStocks]);

    const setItem = (index, key, value, selected) => {
        form.setData('items', form.data.items.map((item, itemIndex) => itemIndex === index
            ? { ...item, [key]: value, satuan: key === 'site_material_stock_id' ? selected?.satuan ?? '' : item.satuan }
            : item));
    };
    ...
    return (
        <>
            ...
            <div className="grid gap-6">
                <Form collapsible title={editing ? `Edit ${editing.kode_pemakaian}` : 'Catat Pemakaian Material'} description="Pemakaian wajib dihubungkan ke progress yang telah disetujui agar konsumsi material dan pekerjaan fisik dapat dibandingkan." onSubmit={submit} actions={<>{editing && <Button type="button" variant="outline" onClick={resetForm}><X size={15} /> Batal</Button>}<Button type="submit" disabled={form.processing}><PackageCheck size={17} /> {editing ? 'Simpan Perubahan' : 'Simpan Pemakaian'}</Button></>}>
                    ...
                    <div className="grid gap-3">
                        <div className="flex items-center justify-between"><h3 className="font-extrabold">Material Terpakai</h3><Button type="button" size="sm" variant="outline" onClick={() => form.setData('items', [...form.data.items, itemTemplate()])}><PlusCircle size={15} /> Tambah Item</Button></div>
                        {form.data.items.map((item, index) => (
                            <div className="grid gap-3 rounded-lg border border-silver-deep/70 p-3 md:grid-cols-[1fr_180px_auto]" key={index}>
                                <div className="grid gap-2"><span className="text-sm font-extrabold">Stok Lokasi</span><Dropdown label="Pilih Material" value={item.site_material_stock_id} options={stockOptions} onChange={(value, selected) => setItem(index, 'site_material_stock_id', value, selected)} /></div>
                                <Input label={`Qty (${item.satuan || '-'})`} type="number" min="0.01" step="0.01" value={item.qty} onChange={(event) => setItem(index, 'qty', event.target.value)} />
                                <div className="flex items-end"><Button type="button" variant="ghost" disabled={form.data.items.length === 1} onClick={() => form.setData('items', form.data.items.filter((_, itemIndex) => itemIndex !== index))}><MinusCircle size={17} /></Button></div>
                            </div>
                        ))}
                    </div>
                ...
                <section className="overflow-hidden rounded-lg border border-white/80 bg-white/78 shadow-soft dark:border-white/10 dark:bg-white/8">
                    <div className="border-b border-silver-deep/60 p-5">
                        <h2 className="text-lg font-extrabold">Saldo Material di Lokasi</h2>
                        <p className="mt-1 text-sm text-ink-soft">Jejak material dari gudang sampai dipakai atau dikembalikan.</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead className="bg-silver-soft/80 text-left text-xs uppercase tracking-wider text-ink-soft"><tr>{['Lokasi', 'Tahapan', 'Material / HPP', 'Diterima', 'Dipakai', 'Menunggu Kembali', 'Dikembalikan', 'Sisa'].map((label) => <th className="px-5 py-4" key={label}>{label}</th>)}</tr></thead>
                            <tbody className="divide-y divide-silver-deep/50">{siteStockRows.map((row) => <tr key={row.id}>
                                ...
                                <td className="px-5 py-4">{row.diterima} {row.satuan}</td><td className="px-5 py-4">{row.dipakai} {row.satuan}</td><td className="px-5 py-4">{row.menunggu_pengembalian} {row.satuan}</td><td className="px-5 py-4">{row.dikembalikan} {row.satuan}</td><td className="px-5 py-4 font-extrabold">{row.sisa} {row.satuan}</td>
                            </tr>)}</tbody>
                        </table>
                    </div>
                </section>
                ...
                <section className="overflow-hidden rounded-lg border border-white/80 bg-white/78 shadow-soft dark:border-white/10 dark:bg-white/8">
                    <form className="grid gap-4 p-5 md:grid-cols-[1.2fr_1fr_1fr_1fr_auto]" onSubmit={filterRows}>
                        <Input label="Cari Pemakaian" value={search} onChange={(event) => setSearch(event.target.value)} />
                        <div className="grid gap-2">
                            <span className="text-sm font-extrabold">Filter Perumahan</span>
                            <Dropdown
                        ...
                        <div className="grid gap-2">
                            <span className="text-sm font-extrabold">Filter Unit</span>
                            <Dropdown label="Semua Unit" value={filterUnit} options={filterUnitOptions} onChange={setFilterUnit} />
                        </div>
                        <div className="grid gap-2">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead className="bg-silver-soft/80 text-left text-xs uppercase tracking-wider text-ink-soft"><tr>{['Kode', 'Tanggal', 'Lokasi', 'Tahapan / Progress', 'Material', 'Bukti', 'Audit', 'Lock', 'Aksi'].map((label) => <th className="px-5 py-4" key={label}>{label}</th>)}</tr></thead>
                            <tbody className="divide-y divide-silver-deep/50">
                                {rows.data.map((row) => <tr key={row.id}>
                                    <td className="px-5 py-4 font-bold">{row.kode_pemakaian}</td><td className="px-5 py-4">{row.tanggal}</td>
                                    <td className="px-5 py-4">{row.perumahan}<br /><span className="text-xs text-ink-soft">{row.unit}</span></td>
                                    <td className="px-5 py-4">{row.tahapan}<br /><span className="text-xs text-ink-soft">{row.progress}</span></td>
                                    <td className="px-5 py-4">{row.items_text}</td>
                                    <td className="px-5 py-4">{row.foto_url ? <a className="font-bold text-emerald-600" href={row.foto_url} target="_blank" rel="noreferrer">Lihat</a> : '-'}</td>
            ...
            <Modal
                ...
                title={detail ? `Detail ${detail.kode_pemakaian}` : 'Detail Pemakaian Material'}
                    ...
                    <div className="grid gap-4 text-sm">
                        <div className="grid gap-3 md:grid-cols-2">
                            <div className="rounded-lg border border-silver-deep/70 p-4">
                                <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Tanggal</p>
                                <p className="mt-1 font-extrabold">{detail.tanggal}</p>
                            </div>
                            <div className="rounded-lg border border-silver-deep/70 p-4">
                                <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Lock</p>
                                <p className="mt-1 font-extrabold">{detail.record_status}</p>
                            </div>
                            <div className="rounded-lg border border-silver-deep/70 p-4">
                                <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Lokasi</p>
                                <p className="mt-1 font-extrabold">{detail.perumahan}</p>
                                <p className="text-ink-soft">{detail.unit}</p>
                            </div>
                            <div className="rounded-lg border border-silver-deep/70 p-4">
                                <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Tahapan / Progress</p>
                                <p className="mt-1 font-extrabold">{detail.tahapan}</p>
                                <p className="text-ink-soft">{detail.progress}</p>
                            </div>
                        </div>
                        <div className="rounded-lg border border-silver-deep/70 p-4">
                            <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Material Terpakai</p>
                            <p className="mt-2 font-bold">{detail.items_text || '-'}</p>
                        </div>
                        <div className="rounded-lg border border-silver-deep/70 p-4">
                            <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Catatan</p>
                            <p className="mt-2 whitespace-pre-line">{detail.keterangan || '-'}</p>
                        </div>
                        <div className="rounded-lg border border-silver-deep/70 p-4">
                            <p className="text-xs font-extrabold uppercase tracking-wider text-ink-soft">Audit</p>
                            <p className="mt-2"><span className="font-bold">Dibuat:</span> {detail.created_by_name}</p>
                            <p><span className="font-bold">Diubah:</span> {detail.updated_by_name}</p>
                        </div>
                        {detail.foto_url && (
                            <a className="font-extrabold text-emerald-600" href={detail.foto_url} target="_blank" rel="noreferrer">
                                Lihat Bukti Foto
                            </a>
                        )}
                    </div>
...
Index.layout = (page) => <AdminLayout title={page?.props?.title ?? 'Pemakaian Material'}>{page}</AdminLayout>;
