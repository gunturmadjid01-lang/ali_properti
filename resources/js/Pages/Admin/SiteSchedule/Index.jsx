    ...
    return (
        <>
            ...
            <div className="grid gap-6">
                <Form collapsible title={editing ? `Edit ${editing.kode_jadwal}` : 'Rencana Kerja Jangka Pendek'} description="Target dibuat dari jadwal, realisasi otomatis mengikuti progress lapangan yang sudah disetujui." onSubmit={submit} actions={<>{editing && <Button type="button" variant="outline" onClick={resetForm}><X size={14} /> Batal</Button>}<Button type="submit"><CalendarClock size={17} /> {editing ? 'Simpan Perubahan' : 'Simpan Jadwal'}</Button></>}>
                    ...
                    <div className={`grid gap-4 ${editing ? 'md:grid-cols-2' : ''}`}>{editing && <Textarea label="Kendala" value={form.data.kendala} onChange={(event) => form.setData('kendala', event.target.value)} />}<Textarea label="Catatan" value={form.data.catatan} onChange={(event) => form.setData('catatan', event.target.value)} /></div>
                </Form>
