...
export default function Index({
    ...
    const destroyRow = (row) => {
        if (window.confirm(`Hapus template ${row.nama_template}?`)) {
            router.delete(`${baseUrl}/${row.id}?context=${selectedContext}`, {
                preserveScroll: true,
            });
        }
    };
        ...
        <>
            ...
            <div className="grid gap-6">
                ...
                <section className="overflow-hidden rounded-lg border border-white/80 bg-white/78 shadow-soft dark:border-white/10 dark:bg-white/8">
                    <form
                        className="grid gap-4 border-b border-silver-deep/60 p-5 dark:border-white/10 md:grid-cols-[220px_1fr_auto] md:items-end"
                        onSubmit={(event) => {
                    ...
                    >
                        ...
                        <Input
                            label="Pencarian"
                            value={search}
                            placeholder="Cari nama template atau perumahan..."
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-silver-deep/60 text-sm dark:divide-white/10">
                            ...
                            <tbody className="divide-y divide-silver-deep/50 dark:divide-white/10">
                                {(rows.data ?? []).map((row) => (
                                    <tr
                                        className="transition hover:bg-silver-soft/60 dark:hover:bg-white/5"
                                        key={row.id}
                                    >
...
Index.layout = (page) => (
    <AdminLayout title={page?.props?.title ?? "Template Pekerjaan SPK"}>
        {page}
    </AdminLayout>
);
