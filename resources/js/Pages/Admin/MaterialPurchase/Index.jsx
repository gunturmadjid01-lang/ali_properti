...
export default function Index({
    ...
    const filter = (event) => {
        ...
        router.get(
            baseUrl,
            { search, gudang_id: gudangId, days, sort, direction },
    ...
    const lockRow = (row) =>
        window.confirm(`Lock pembelian ${row.kode_pembelian}?`) &&
        post(`${baseUrl}/${row.id}/lock`);
    const unlockRow = (row) =>
        window.confirm(`Buka lock pembelian ${row.kode_pembelian}?`) &&
        post(`${baseUrl}/${row.id}/unlock`);
    const approveRow = (row) =>
        window.confirm(`Approve pembelian ${row.kode_pembelian}?`) &&
        post(`${baseUrl}/${row.id}/approve`);

    const workflowButtons = (row) => {
        if (row.record_status === "locked") {
            return row.can_unlock && permissions.canUnlock ? (
                <IconButton
                    title="Buka Kunci"
                    icon={Unlock}
        ...
        return (
            <>
                {permissions.canApprove && row.can_approve && (
                    <IconButton
                        title="Setujui"
                        icon={CheckCircle2}
                ...
                {permissions.canLock && row.can_lock && (
                    <IconButton
                        title="Kunci"
                        icon={Lock}
                        onClick={() => lockRow(row)}
                    ...
                    <div className="grid gap-4">
                        <div className="grid gap-3 rounded-lg border border-silver-deep/60 bg-silver-soft/60 p-4 dark:border-white/10 dark:bg-white/5 md:grid-cols-5">
                            ...
                            <div>
                                ...
                                <p className="mt-1 font-black">
                                    {detail.metode_pembayaran === "hutang"
                                        ? "Hutang Supplier"
                                        : "Tunai"}
                                </p>
                            </div>
                        ...
                        <div className="grid gap-2 md:ml-auto md:w-96">
                            <TotalRow
                                label="Sub Total"
                                value={
                                    detail.subtotal_nominal ??
                                    detail.total_nominal
                                }
                            />
                            <TotalRow
                                label="Potongan Transaksi"
                                value={detail.diskon_transaksi ?? 0}
                            />
                            <TotalRow
                                label="Total Akhir"
                                value={detail.total_nominal}
                                strong
                            />
                        </div>
                    </div>
...
function TotalRow({ label, value, strong = false }) {
    return (
        <div
            className={`grid grid-cols-[1fr_160px] items-center gap-3 text-sm ${strong ? "font-black" : "font-bold"}`}
        >
...
Index.layout = (page) => (
    <AdminLayout title={page?.props?.title ?? "Daftar Pembelian Material"}>
        {page}
    </AdminLayout>
);
