...
export default function Inspection({
    ...
    const inspect = (purchase, item, inspectionStatus, forcedQty = null) => {
        ...
        const qtyDiterima =
            forcedQty !== null
                ? forcedQty
                : inspectionStatus === "sesuai"
                  ? item.qty
                  : Number(input.qty_diterima ?? 0);
        const label =
            qtyDiterima > 0 && qtyDiterima < Number(item.qty)
                ? `terima sebagian ${qtyDiterima} ${item.satuan}`
                : inspectionStatus === "sesuai"
                  ? "terima sesuai"
                  : "tolak/tidak sesuai";
        ...
        if (
            !window.confirm(
                `Proses ${item.barang}: ${label}? Keputusan ini tidak dapat diproses dua kali.`,
            )
        )
            return;

        router.post(
            `${baseUrl}/${purchase.id}/item/${item.id}`,
            {
            ...
            <div className="grid gap-5">
                <section className="rounded-lg border border-white/80 bg-white/78 p-5 shadow-soft dark:border-white/10 dark:bg-white/8">
                    ...
                    <p className="mt-1 text-sm font-medium text-ink-soft">
                        Periksa pembelian per item. Hanya item yang dinyatakan
                        sesuai yang masuk ke stok gudang.
                    </p>
                    <form
                        className="mt-5 grid gap-3 lg:grid-cols-[1.4fr_0.8fr_0.8fr_1fr_1fr_auto]"
                        onSubmit={applyFilter}
                    >
                        <Input
                            label="Cari Kode Pembelian / Supplier"
                            value={search}
                                    ...
                                    <tr key={purchase.id}>
                                        ...
                                        <td className="px-5 py-4 text-xs font-bold">
                                            <span className="text-emerald-600">
                                                {purchase.accepted_count} sesuai
                                            </span>
                                            <span className="mx-2 text-ink-soft">
                                                /
                                            </span>
                                            <span className="text-red-500">
                                                {purchase.rejected_count} tidak
                                            </span>
                                            <span className="mx-2 text-ink-soft">
                                                /
                                            </span>
                                ...
                                {rows.data.length === 0 && (
                                    <tr>
                                        <td
                                            ...
                                            colSpan={10}
                                        >
                                            Belum ada pembelian yang perlu
                                            diperiksa.
                                        </td>
                                    </tr>
                                )}
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                "pending" ? (
                                                    <div className="grid min-w-72 gap-2">
                                                        ...
                                                        <TableActions
                                                            label={`Aksi pemeriksaan ${item.barang}`}
                                                        >
                                                            <Button
                                                                type="button"
                                                                size="sm"
