...
test('spk tukang sendiri menyimpan item pekerjaan dan pembayaran terhubung ke item spk', function () {
    ...
    expect($spk->sumber_tenaga_kerja)->toBe('tukang_owner')
        ->and($spk->kontraktor?->nama_kontraktor)->toBe('Tukang Sendiri')
        ->and($spk->nilai_kontrak_dasar)->toBe(70000.0)
        ->and($spk->nilai_kontrak)->toBe(70000.0)
        ->and($item->nama_tahap_pekerjaan)->toBe('PEKERJAAN STRUKTUR')
        ->and($item->nama_pekerjaan)->toBe('Pasang Bata Merah')
        ->and($payment->pekerjaan)->toBe('PEKERJAAN STRUKTUR - Pasang Bata Merah')
        ->and($payment->spk_kontraktor_item_id)->toBe($item->id);
});
...
test('spk yang disetujui menambah item baru di rab perumahan bagian iv', function () {
    ...
    expect((float) $item->jumlah_rab)->toBe(50000.0)
        ->and((float) $item->harga_satuan)->toBe(50000.0)
